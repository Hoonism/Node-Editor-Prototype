...
class QDMGraphicsView(QGraphicsView):
    ...
    def leftMouseButtonPress(self, event):
        ...
        if type(item) is QDMGraphicsSocket:
            print(item.socket.socket_type)
            if self.mode == MODE_NOOP and (len(item.socket.edges) == 0 or item.socket.socket_type == "output"):
                self.mode = MODE_EDGE_DRAG
            elif self.mode == MODE_NOOP and len(item.socket.edges) > 0 and item.socket.socket_type == "input":
    ...
    def edgeDragStart(self, item):
        ...
        if self.mode == MODE_EDGE_REASSIGN:
            if item.socket.edges[0].start_socket.socket_type == "output":
                self.drag_start_socket = item.socket.edges[0].start_socket
            if item.socket.edges[0].end_socket.socket_type == "output":
                self.drag_start_socket = item.socket.edges[0].end_socket
            self.new_edge = item.socket.edges[0]
            self.new_edge.remove()
            self.new_edge = None
            self.drag_edge = Edge(self.grScene.scene, self.drag_start_socket, None)
    ...
    def edgeDragEnd(self, item):
        ...
        if type(item) is QDMGraphicsSocket:
            ...
            if (last_mode == 3 or id(item) != id(self.drag_start_socket)) and item.socket.socket_type != self.drag_start_socket.socket_type:
                ...
                self.new_edge = Edge(self.grScene.scene, self.drag_start_socket, item.socket)
                if DEBUG:
