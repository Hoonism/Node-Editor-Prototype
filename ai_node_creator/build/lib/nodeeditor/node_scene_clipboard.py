...
class SceneClipboard():
    ...
    def serializeSelected(self, delete=False):
        ...
        sel_nodes, sel_edges, socket_edge, sel_sockets = [], [], [], {}
        ...
        for item in self.scene.grScene.selectedItems():
            if hasattr(item, 'node'):
                ...
                for socket in (item.node.inputs + item.node.outputs):
                    ...
                    socket_edge.append(socket.edges)
            elif isinstance(item, QDMGraphicsEdge):
                sel_edges.append(item.edge)
        edge_appearance = Counter(socket_edge)
        for item in edge_appearance.items():
            if item[1] == 2:
                sel_edges.append(item[0])
        ...
        # if CUT {aka delete} remove selected item
        if delete:
    ...
    def deserializeFromClipboard(self, data):
        print('deserializing from clipboard, data:', data)
        ...
        # calculate selected objects bounding box & center
        ...
        # calcualte the offset of the newly created nodes
